package com.test.dspc_improved;
...
public class DPSC_improved {

    //minimum support count
    private final static int min_sup = 200;
    ...
    public static class MapPhase2 extends Mapper<LongWritable, Text, Text, IntWritable>
    {
        ...
        private Set<Integer> createSILsequence(boolean itemEx, Set<Integer> X, Set<Integer> Y)
        {
            Set<Integer> Z = new HashSet<>();
            Iterator<Integer> x = X.iterator();
            Iterator<Integer> y = Y.iterator();
            if(itemEx)
            {
                int a = x.next();
                int b = y.next();
        ...
        private void generateSequences(String Seq, HashMap<String,Set<Integer>> SIL, Context context) throws IOException, InterruptedException
        {
            if(Seq.length()<=6)
            {
                return;
            ...
            if(CMAPs.containsKey(lastitem))
            {
                for (String itemc : CMAPs.get(lastitem))
                {
                    ...
                    {
                        Set<Integer> SILnewSeq = createSILsequence(false,SIL.get(Seq),SIL.get(itemc+" -1 -2"));
                        Integer count = SILnewSeq.size();
                        if(!SILnewSeq.isEmpty())
                        {
            ...
            if(CMAPi.containsKey(lastitem))
            {
                for(String itemc: CMAPi.get(lastitem))
                {
                    ...
                    if(!earlyPrune(newSeq,itemc))
                    {
                        Set<Integer> SILnewSeq = createSILsequence(true,SIL.get(Seq),SIL.get(itemc+" -1 -2"));
                        Integer count = SILnewSeq.size();
                        if(!SILnewSeq.isEmpty())
                        {
                            SIL.put(newSeq,SILnewSeq);
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
        {
            ...
            {
                String item = tokenizer.nextToken()+" -1 -2";
            ...
            HashMap<String,Set<Integer>> SILcpy = new HashMap<>();
            for(HashMap.Entry<String,Set<Integer>> entry: itemPosMapping.entrySet())
            {
                SILcpy.put(entry.getKey(),entry.getValue());
            }
            for(HashMap.Entry<String,Set<Integer>> entry: SILcpy.entrySet())
            {
                String k = entry.getKey();
                generateSequences(k, itemPosMapping, context);
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        job1.setJarByClass(DPSC_improved.class);
        ...
        job2.setJarByClass(DPSC_improved.class);
